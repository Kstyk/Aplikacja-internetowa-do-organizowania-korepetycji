...
const PurchaseTeacherHistoryTable = (props) => {
        ...
        {purchases?.map((purchase) => (
          ...
          >
            ...
            <div className="flex w-20 items-center justify-center px-2 py-3 text-center sm:w-24 sm:p-3">
              <Link
                className="transition-all duration-200 hover:underline sm:truncate"
                to={`/student/${purchase?.student?.id}`}
              >
                {purchase?.student?.first_name} {purchase?.student?.last_name}
              </Link>
            </div>
...
export default PurchaseTeacherHistoryTable
